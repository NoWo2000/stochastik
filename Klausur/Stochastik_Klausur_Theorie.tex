...
$n$ bezeichnet die Gesamtzahl der Objekte und $k$ die Anzahl der Objekte, die ausgewählt wurden.
\begin{itemize}
...
& \\
& \text{----- Zusatz zum Verstehen -----} \\
& \\
& = \sum_{j=1}^{n} \frac{P(A \cap B)}{P(B_j)} \cdot P(B_j)  \\
P(B_j) \text{ kann gekürzt werden }& = \sum_{j=1}^{n} {P(A \cap B_j)} \\
& \Rightarrow \text{ Man nimmt sich aus jedem } P(A \cap B_1) \text{ bis } P(A \cap B_n) \\
& \text{die Anteile von A und summiert sie auf, denn man nimm } \\
& \text{die Summe über 1 bis n für B, was dann wieder} \\
& \text{der Menge von } \Omega \text{ entspricht.} \\
\end{align*}
